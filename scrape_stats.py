...
def find_opponent_raw_value(stat_dict, index):
    ...
    opponent_player_number = tournament_constants.PLAYER_NUMBERS[opponent_index]
...
def fill_match_stat_for_player(stat_dict, stats_array):
    for index, player_number in enumerate(tournament_constants.PLAYER_NUMBERS):
        ...
        if stat_name == "Break Points Won":
            ...
            player_dict["Break Points Saved"] = int(
                opponent_raw_value.split("/")[1].split(" ")[0]
            ) - int(opponent_break_points_won)
